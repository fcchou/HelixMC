...
cdef inline void normal_vec(double* r, double* rp) nogil:
    ...
    if is_vec_nil(rp):
        rp[0] = 0; rp[1] = 0; rp[2] = 1 #Since r is parallel to ex, just take ez as normal vector
##########################################################
...
cdef double _writhe_int(double[:,::1] r) nogil:
    ...
    for i in xrange(2, nr-1):
        for j in xrange(i-1):
            ...
            #Normalize the normal vectors. If any normal vectors is 0, just continue.
            is_zero = False
            for k in xrange(4):
                if is_vec_nil(n[k]):
                    is_zero = True
